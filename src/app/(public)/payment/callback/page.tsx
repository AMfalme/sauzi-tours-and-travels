...
export default function PaymentCallbackPage() {
  ...
  const txRef = paystackReference || trxref || searchParams.get("tx_ref") || "";

  const isSuccess = status.toLowerCase() === "success" || status.toLowerCase() === "successful";

  return (
    <section className="mx-auto max-w-2xl px-4 py-16">
      <div className={`rounded-2xl border p-6 ${isSuccess ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}>
        ...
        {txRef ? (
          <p className="mt-3 text-xs text-slate-600">Reference: {txRef}</p>
        ) : null}

        <div className="mt-6 flex gap-3">
          <Link href="/" className="rounded-lg bg-brand-primary px-4 py-2 text-sm font-semibold text-white">
            Back to Home
          </Link>
